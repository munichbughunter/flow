...
var (
	PipelinePath = "/var/flow/pipeline"
	StatePath    = "/var/flow-state"
	FlowVolume   = &yaml.Volume{
		Name:     "flow",
	...
	FlowStateVolume = &yaml.Volume{
		Name:     "flow-state",
		EmptyDir: &yaml.VolumeEmptyDir{},
	}
	FlowVolumeMount = &yaml.VolumeMount{
		Name:      "flow",
		MountPath: "/var/flow",
	}
	FlowStateVolumeMount = &yaml.VolumeMount{
		Name:      "flow-state",
		MountPath: StatePath,
	}
)
...
func (c *Client) newPipeline(opts newPipelineOpts, pipelineOpts clients.CommonOpts) *yaml.Pipeline {
	...
	build := &yaml.Container{
		...
		Volumes: []*yaml.VolumeMount{FlowVolumeMount},
	}
	...
	for i, v := range opts.Steps {
		...
		opts.Steps[i].Volumes = append(opts.Steps[i].Volumes, FlowVolumeMount, FlowStateVolumeMount)
	}
