...
// This allows the flow library to define different methods of retrieving the same information on various clients.
// For example, when running in CLI or Dagger client, getting the git ref might be as simple as running `git rev-parse HEAD`.
...
// These arguments can be provided to the CLI by using the flag `-arg`, for example `-arg=workdir=./example` will set the "workdir" argument to "example" in the CLI client.
