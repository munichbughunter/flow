...
// PipelineArgs are provided to the `flow` command.
type PipelineArgs struct {
	Client string

	// Path is provided in every execution to the flow run command,
	// and contians the user-supplied location of the flow pipeline (or "." / "$PWD") by default.
	Path    string
	Version string
	...
	// Examples:
	// * 'fs:///var/flow/state.json' - Uses a JSON file to store the state.
	// * 'fs:///c:/flow/state.json' - Uses a JSON file to store the state, but on Windows.
	// * 'fs:///var/flow/state/' - Stores the state file in the given directory, using a randomly generated ID to store the state.
	//    * This might be a good option if implementing a Flow client in a provider.
	// * 's3://bucket-name/path'
...
func ParseArguments(args []string) (*PipelineArgs, error) {
	...
	flagSet.StringVar(&version, "version", "latest", "The version is provided by the 'flow' command, however if only using 'go run', it can be provided here")
